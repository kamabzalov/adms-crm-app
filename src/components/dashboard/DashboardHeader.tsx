...
export function DashboardHeader() {
  ...
  const { useruid, loginname }: LoginResponse = JSON.parse(localStorage.getItem('admss-admin-user') ?? '');
  const signOut = () => {
    if (useruid) {
          ...
          <div className="app-header-menu d-flex align-items-stretch w-100">
            <div className="menu menu-rounded menu-column menu-row my-5 my-lg-0 align-items-stretch fw-semibold  px-lg-0 w-100">
              <div className="menu-item me-lg-1">
                <Link className="menu-link py-3 text-hover-primary" to={'/dashboard'}>
                  <i className="ki-outline ki-user-tick fs-2 m-2"></i>
                  <span className="menu-title">Users</span>
                </Link>
              </div>
              <div className="menu-item me-lg-1">
                <Link className="menu-link py-3 text-hover-primary" to={'/dashboard/billing'}>
                  <i className="ki-outline ki-bill fs-2 m-2"></i>
                  <span className="menu-title">Billing</span>
                </Link>
              </div>
              <div className="menu-item me-lg-1 flex-grow-1">
                <Link className="menu-link py-3 text-hover-primary" to={'/dashboard/reports'}>
              ...
              <div className="menu-item me-lg-1">
                <CustomDropdown title={loginname}>
                  <span onClick={() => signOut()} className="menu-link text-hover-primary">
